...
export class PagerView extends BaseView<IPagerProps, PagerViewModel> {
  ...
  render() {
    ...
    let pager = (this.props.limits == null || this.state.itemCount() === 0 || this.state.limit() == null) ? null : (
      <Pagination items={this.state.pageCount()} activePage={this.state.selectedPage()} onSelect={this.bindEvent(x => x.selectPage, (e, x) => x[0].eventKey as number)} {...this.props as any} />
    );

    let limit = (this.props.limits == null || this.state.itemCount() === 0) ? null : (
      <DropdownButton id='pager-limit-selector' title={`Items per Page (${this.state.limit() || 'All'})`} onSelect={this.bindEventToProperty(x => x.limit, (e, x) => x[0])}>
